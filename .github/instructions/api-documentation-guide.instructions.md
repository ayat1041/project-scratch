---
description: "Swagger/OpenAPI documentation standards for backend endpoints: JSDoc patterns, request/response schemas, examples, Postman sync, and file organization."
applyTo: "apps/backend/src/**/*.{swagger.ts,swagger-docs.ts,routes.ts}"
---
...
1. **Create per-endpoint files under a `swagger-docs` folder**
   ...
   ```
   src/modules/user-management/features/F6002-roles/
    ├── controllers/
    ├── services/
    ├── validations/
    ├── roles.routes.ts
    └── swagger-docs/
      ...
      ├── assign-role-permissions.swagger.ts
      └── index.ts
   ```
...
tags:
  - User Management - Roles - Permission Assignment
```
...
const targets = [
  '/api/user-management/v1/roles/search',
...
You should see expected module folders (for example `Roles`) based on your tags.
...
## AI Generation Checklist (for AI-assisted generation)
...
> - Tag format must match desired Postman folder structure (e.g. `User Management - Role Permission Assignment`)
